...
#' Bandwidth selection for local Gaussian correlation (using parallelization).
#'
#' Takes a matrix of data points and returns the bandwidths used for estimating
#' the local Gaussian correlations.
#'
...
bw_select_par <- function(x,
  ...
  if(est_method == "5par_marginals_fixed") {
    ...
    for(i in 1:d) { # This is fast, so do not bother to parallelize
      if(bw_method == "cv") {
        result <- bw_select_cv_univariate(x[, i], tol = tol_marginal)
        marginal_bandwidths[i] <- result$bw
        marginal_convergence[i] <- result$convergence
  ...
  # Iterate over all the pairs
  joint_bandwidths2 <- foreach(i=1:nrow(joint_bandwidths),.packages="lg",.combine=rbind) %dopar% {
    ...
    if(bw_method == "cv") {
      ...
      # Replace these by the lines below
      #joint_bandwidths$bw1[i] <- result$bw[1]
      #joint_bandwidths$bw2[i] <- result$bw[2]
      #joint_bandwidths$convergence[i] <- result$convergence

      # Use these instead
      res.list$bw1 <- result$bw[1]
      res.list$bw2 <- result$bw[2]
      res.list$convergence <- result$convergence
      ...
      res.list$bw1 <- bw
      res.list$bw2 <- bw
      res.list$convergence <- NA
    }

    unlist(res.list) # Returned

  }

  # Putting the results from the foreach-loop in appropriate format
  joint_bandwidths[,c("bw1","bw2","convergence")]=as.data.frame(joint_bandwidths2,row.names = NULL)
