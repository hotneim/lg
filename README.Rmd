...
We can then specify a set of grid points and estimate the probability density function of *x* using the *dlg()*-function. We choose a set of grid points that go diagonally through R^4, estimate, and plot the result as follows:
...
the bandwidths and so in the call to *lg_main()*, and there are further options available in *ind_test()*. For this illustration we use only 20 bootstrap replications, but this must of course be significantly higher in practical applications.
