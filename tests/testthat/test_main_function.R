...
test_that("The function produces the correct errors", {
    expect_error(lg(data_bad), "The data must be a matrix or a data frame")
    expect_error(lg(data_trivar, est_method = est_method_bad))
    expect_error(lg(data_trivar, est_method = "5par"),
                 "Data must be bivariate if estimation method is '5par'")
    expect_warning(lg(data_trivar,
                      est_method = "1par",
